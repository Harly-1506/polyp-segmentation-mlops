...
from kfp.dsl import Metrics, Model, OutputPath

BASE_IMAGE = "harly1506/polyp-mlops:kfpv4"
DEPLOY_BASE_IMAGE = "python:3.12-slim"
EXPORT_PACKAGES = [
    "torch==2.7.1",
    "torchvision==0.22.1",
...
@dsl.component(
    ...
    packages_to_install=EXPORT_PACKAGES,
)
def export_onnx_component(
    ...
    """Download a checkpoint, export it to ONNX, and upload the artifact."""
    import os
    import subprocess
    import sys
    import tempfile
    ...
    if checkpoint_uri.startswith("gs://"):
        ...
        bucket_name, *path_parts = checkpoint_uri[5:].split("/", 1)
    ...
    model = torch.jit.load(checkpoint_path) if checkpoint_path.suffix == ".pt" else None
    if model is None:
        try:
            ...
            if not repo_url:
                raise RuntimeError(
                    "training.models not found. Set TRAINING_REPO_URL to a git repository containing the model definition."
                )
    ...
    if output_uri.startswith("gs://"):
        bucket_name, *path_parts = output_uri[5:].split("/", 1)
        blob_path = path_parts[0]
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob = bucket.blob(blob_path)
...
def trigger_jenkins_component(
    ...
    """Invoke a Jenkins job to promote a new model version."""
    import requests
    ...
    if not jenkins_token:
        raise ValueError("jenkins_token must be provided to trigger the Jenkins job")
...
def evaluate_model_component(
    ...
    _Path(quality_gate).write_text("true" if bool(passed) else "false", encoding="utf-8")
...
def ray_segmentation_training_pipeline(
    ...
    deploy_checkpoint_uri: str = "",
    deploy_onnx_uri: str = "",
    deploy_target_env: str = "",
    deploy_model_name: str = "",
    deploy_model_version: str = "",
    deploy_canary_percent: int = 25,
    jenkins_url: str = "http://polyp-jenkins.jenkins:8080",
    jenkins_user: str = "admin",
    jenkins_token: str = "",
    jenkins_job_name: str = "polyp-canary-promotion",
):
    ...
    with dsl.Condition(enable_deployment == True):  # pylint: disable=singleton-comparison
        with dsl.Condition(evaluation_task.outputs["quality_gate"] == "true"):
            with dsl.Condition(deploy_checkpoint_uri != ""):
                with dsl.Condition(deploy_onnx_uri != ""):
                    export_task = export_onnx_component(
                        checkpoint_uri=deploy_checkpoint_uri,
                        output_uri=deploy_onnx_uri,
                    )
                    with dsl.Condition(deploy_target_env != ""):
                        with dsl.Condition(jenkins_token != ""):
                            trigger_jenkins_component(
                                ...
                                model_version=deploy_model_version,
                                canary_percent=deploy_canary_percent,
                                target_env=deploy_target_env,
    )


@dsl.pipeline(name="polyp-segmentation-deploy")
def segmentation_deploy_pipeline(
    checkpoint_uri: str,
    onnx_uri: str,
    model_name: str,
    model_version: str,
    ...
    with dsl.Condition(evaluation_passed == True):  # pylint: disable=singleton-comparison
        with dsl.Condition(jenkins_token != ""):
            trigger_jenkins_component(
                jenkins_url=jenkins_url,
                jenkins_user=jenkins_user,
...
PIPELINE_REGISTRY: Dict[str, Callable[..., None]] = {
    "training": ray_segmentation_training_pipeline,
    "deploy": segmentation_deploy_pipeline,
}


def compile_pipeline(output_path: str, pipeline: str = "training") -> None:
    try:
        pipeline_func = PIPELINE_REGISTRY[pipeline]
    except KeyError as exc:
        raise ValueError(
            f"Unknown pipeline '{pipeline}'. Available options: {sorted(PIPELINE_REGISTRY)}"
        ) from exc
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="ray_segmentation_pipeline_v10.yaml",
    ...
    parser.add_argument(
        ...
        default="training",
        help="Which pipeline to compile (training or deploy).",
    )
    args = parser.parse_args()
    compile_pipeline(args.output, pipeline=args.pipeline)
    print(
        f"Pipeline '{args.pipeline}' YAML written to {args.output}"
    )